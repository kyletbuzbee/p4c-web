...
const AdminDashboard: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const occupiedCount = properties.filter(p => p.status !== 'available').length; // Adjust logic as needed
    ...
    const estimatedRevenue = properties.reduce((acc, curr) => acc + (parseInt(curr.price.replace(/,/g, '')) || 0), 0);
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button 
            ...
            className={`flex items-center w-full px-4 py-3 rounded-lg font-medium transition-colors ${
              activeView === 'dashboard' ? 'bg-white/10 text-p4c-gold' : 'text-gray-300 hover:bg-white/5'
            }`}
          >
          ...
          <button 
            ...
            className={`flex items-center w-full px-4 py-3 rounded-lg font-medium transition-colors ${
              activeView === 'properties' ? 'bg-white/10 text-p4c-gold' : 'text-gray-300 hover:bg-white/5'
            }`}
          >
          ...
          <button 
            ...
            className={`flex items-center w-full px-4 py-3 rounded-lg font-medium transition-colors ${
              activeView === 'tenants' ? 'bg-white/10 text-p4c-gold' : 'text-gray-300 hover:bg-white/5'
            }`}
          >
          ...
          <button 
             ...
             className={`flex items-center w-full px-4 py-3 rounded-lg font-medium transition-colors ${
              activeView === 'settings' ? 'bg-white/10 text-p4c-gold' : 'text-gray-300 hover:bg-white/5'
            }`}
          >
      ...
      <main className="flex-1 overflow-y-auto md:ml-64 transition-all">
        <div className="p-8">
          ...
          {activeView === 'dashboard' && (
            <>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                ...
                <p className="text-gray-500 text-sm">No recent alerts or system notifications.</p>
              </div>
            </>
          )}
          ...
          {(activeView === 'dashboard' || activeView === 'properties') && (
            <div className={`bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden ${activeView === 'dashboard' ? 'mt-8' : ''}`}>
              <div className="px-6 py-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                <h3 className="font-bold text-gray-800">
                  {activeView === 'dashboard' ? 'Recent Listings' : 'All Properties'}
                </h3>
                ...
                {activeView === 'dashboard' && (
                  <button onClick={() => setActiveView('properties')} className="text-sm text-p4c-gold font-medium hover:underline">
                    View All
                  </button>
                )}
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {(activeView === 'dashboard' ? filteredProperties.slice(0, 5) : filteredProperties).map((property) => (
          ...
          {activeView === 'tenants' && (
             <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
               ...
               <p className="text-gray-500">Tenant list and application processing features coming soon.</p>
             </div>
          )}
           ...
           {activeView === 'settings' && (
             <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
               ...
               <p className="text-gray-500">Global configurations will be available here.</p>
             </div>
          )}
