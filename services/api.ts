  ...
  impact: {
    ...
    getFinancialBreakdown: async (): Promise<FinancialBreakdown[]> => {
      try {
        // Try backend first if in backend mode
        if (API_CONFIG.isBackendMode) {
          const data = await apiClient.get<FinancialBreakdown[]>('/api/impact/financial-breakdown');
          return data;
        }
      } catch (error) {
        console.warn('Backend financial breakdown request failed, using mock data:', error);
      }
  ...
  transparency: {
    ...
    getStandards: async (): Promise<RenovationStandard[]> => {
      try {
        // Try backend first if in backend mode
        if (API_CONFIG.isBackendMode) {
          const data = await apiClient.get<RenovationStandard[]>('/api/transparency/standards');
          return data;
        }
      } catch (error) {
        console.warn('Backend standards request failed, using mock data:', error);
      }
  ...
  properties: {
    ...
    getAll: async (): Promise<ExtendedProperty[]> => {
      try {
        // Try backend first if in backend mode
        if (API_CONFIG.isBackendMode) {
          const data = await apiClient.get<ExtendedProperty[]>('/api/properties');
          return data;
        }
      } catch (error) {
        console.warn('Backend properties request failed, using mock data:', error);
      }
    ...
    getById: async (id: string): Promise<ExtendedProperty | null> => {
      try {
        // Try backend first if in backend mode
        if (API_CONFIG.isBackendMode) {
          const data = await apiClient.get<ExtendedProperty>(`/api/properties/${id}`);
          return data;
        }
      } catch (error) {
        console.warn('Backend property by ID request failed, using mock data:', error);
      }
    ...
    search: async (criteria: {
      ...
      try {
        // Try backend first if in backend mode
        if (API_CONFIG.isBackendMode) {
          const data = await apiClient.post<ExtendedProperty[]>('/api/properties/search', criteria);
          return data;
        }
      } catch (error) {
        console.warn('Backend property search request failed, using mock data:', error);
      }
    ...
    getByBadge: async (badge: string): Promise<ExtendedProperty[]> => {
      try {
        // Try backend first if in backend mode
        if (API_CONFIG.isBackendMode) {
          const data = await apiClient.get<ExtendedProperty[]>(`/api/properties/badge/${encodeURIComponent(badge)}`);
          return data;
        }
      } catch (error) {
        console.warn('Backend property by badge request failed, using mock data:', error);
      }
