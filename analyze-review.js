...
const imageViolations = review.analysis.files.filter(file => 
  file.violations.some(violation => 
    violation.type === 'ACCESSIBILITY' && violation.message.includes('Image missing alt text')
  )
);
imageViolations.forEach(file => {
  console.log(`- ${file.path}`);
  file.violations.filter(violation => 
    violation.type === 'ACCESSIBILITY' && violation.message.includes('Image missing alt text')
  ).forEach(violation => {
...
const buttonViolations = review.analysis.files.filter(file => 
  file.violations.some(violation => 
    violation.type === 'ACCESSIBILITY' && violation.message.includes('Icon button missing aria-label')
  )
);
